defmodule Refactoring.Cap01.FirstExample.Step23 do
  def start do
  ...
  def render_plain_text(data) do
    ...
    result =
      Enum.reduce(
        ...
        fn perf, result ->
          result <>
          """
            #{perf["play"]["name"]}: $#{usd(amount_for(perf))} (#{perf["audience"]} seats)
          """
      end)
    ...
    result = result <>
    """
    Amount owed is $#{total_amount(render_plain_text_total_amount_data) |> usd()}
    """
    ...
    result = result <>
    """
    You earned $#{total_volume_credits(render_plain_text_total_volume_credits_data)} credits
    """
  ...
  def total_volume_credits(data) do
    volume_credits = 0
    Enum.reduce(
      data.performances,
      volume_credits,
  ...
  def volume_credits_for(a_performance) do
    result = 0
  ...
  def amount_for(a_performance) do
