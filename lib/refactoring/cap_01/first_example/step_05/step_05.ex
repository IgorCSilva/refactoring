defmodule Refactoring.Cap01.FirstExample.Step05 do
  def start do
    ...
    {result, total_amount, volume_credits} =
      Enum.reduce(
        ...
        fn perf, {result, total_amount, volume_credits} ->
          ...
          volume_credits =
            if ("comedy" == play_for(perf, plays)["type"]) do
          ...
          result = result <>
          """
            #{play_for(perf, plays)["name"]}: $#{format.(this_amount / 100)} (#{perf["audience"]} seats)
          """
  ...
  def amount_for(a_performance, play) do
    case play["type"] do
      "tragedy" ->
        result = 40000
